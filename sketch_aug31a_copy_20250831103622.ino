...
const char* update_json_url = "https://raw.githubusercontent.com/kasman121-byte/esp32baubauIot/main/update.json";
...
void checkForUpdate() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    client.setInsecure(); // skip verifikasi SSL, aman untuk testing
    ...
    if (http.begin(client, update_json_url)) {
      ...
      Serial.println("Gagal memulai koneksi ke server JSON.");
    }
  } else {
...
void performOTA(String firmwareURL) {
  WiFiClientSecure client;
  client.setInsecure(); // skip verifikasi SSL
